...
# 추가
def insertOne(groupno, company, period, spec, question, context):    # 시퀀스(int), 회사이름(str), 카테고리(str), 내용(str)
    sql = 'INSERT INTO coverletter VALUES (NEXT VALUE FOR coverletterseq, %s, %s, %s, %s, %s, %s)' # sql / seqno 스트링으로 전달 해야함
    params = (groupno, company, period, spec, question, context)          # 파라미터        
    print("insert 쿼리 전달")
...
def selectOne(seqno):
    ...
    sql = 'SELECT * FROM coverletter WHERE seqno = %s'
