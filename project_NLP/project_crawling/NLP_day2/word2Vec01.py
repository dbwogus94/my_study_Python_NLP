...
def EmbedWrite():
    ...
    model.save("WordToVec02")     # 파일이름 "WordToVec"으로 단어토큰을 백터화(임베딩)시킨 데이터 저장


# Embedding 파일 읽어서 객체생성
def getEmbedding():
    return Word2Vec.load("WordToVec02")   # Word2Vec모듈의 함수 .load()에 인자로 읽어올 파일을 넣어준다.
...
if __name__ == '__main__':
    ...
    model = getEmbedding()
    a = ["도전"]
    print(model.most_similar(positive=["집념"], topn=10))
